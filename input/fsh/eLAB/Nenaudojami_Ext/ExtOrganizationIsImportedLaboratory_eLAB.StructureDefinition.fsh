Extension: ElabExtOrganizationIsImportedLaboratory
Parent: Extension
Id: elab-ext-organization-isimportedlaboratory
Title: "ElabExtOrganizationIsImportedLaboratory"
Description: "Extension indicating that the organization is an imported laboratory."
* url = "http://esveikata.lt/fhir/StructureDefinition/elab-ext-organization-isImportedLaboratory"
* name = "ElabExtOrganizationIsImportedLaboratory"
* status = "active"
* date = "2023-02-09"
* publisher = "Registrų Centras"
